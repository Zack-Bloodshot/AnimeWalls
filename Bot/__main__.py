...
async def danparse():
    try:
        ...
        tag_choices = [
            ...
            "forest",
            "sky",
            f"{random.choice(the_list)}",
        ]
        # tag = f'{random.choice(the_list)}'
        tag = f"{random.choice(tag_choices)}"
        ...
        for post in posts:
            try:
                fu = post["file_url"]
                if check_exist(fu):
                    hashes = await get_dan_hash(
                        post["tag_string_character"], post["tag_string_copyright"]
                    )
                    add_url(fu, "danbooru")
                    dl = down(fu, hashes)
                    res = [dl, hashes, fu]
        ...
        try:
            if len(result) < 3:
                ...
                await bot.send_message(
                    channel, result[1], file=result[0], force_document=True
                )
                os.remove(result[0])
